...
public class AlbumController {
    ...
    public List<String> getCurrent() {
        ...
        Cursor cursor = resolver.query(Media.EXTERNAL_CONTENT_URI, new String[]{ImageColumns.DATA,
                ImageColumns.DATE_ADDED, ImageColumns.SIZE}, ImageColumns.SIZE + " > " + IMAGE_SIZE + " AND (" + MediaStore.Images.Media.MIME_TYPE + "= ? OR "
                + MediaStore.Images.Media.MIME_TYPE + "= ?)", new String[]{"image/jpeg", "image/png"}, ImageColumns.DATE_ADDED + "  DESC");
        if (cursor == null) {
            return photos;
        }
    ...
    public List<String> getAlbum(String name) {
        ...
        Cursor cursor = resolver.query(Media.EXTERNAL_CONTENT_URI, new String[]{ImageColumns.BUCKET_DISPLAY_NAME,
                        ImageColumns.DATA, ImageColumns.DATE_ADDED, ImageColumns.SIZE}, "bucket_display_name = ? AND _size > ? AND (" + MediaStore.Images.Media.MIME_TYPE + "=? OR "
                        + MediaStore.Images.Media.MIME_TYPE + " = ?)",
                new String[]{name, IMAGE_SIZE + "", "image/jpeg", "image/png"}, ImageColumns.DATE_ADDED + " DESC");
        if (cursor == null)
            return photos;
